...
# 步数
scoreboard players operation #steps teleport = @s teleport.backward
scoreboard players operation #steps teleport *= #4 teleport
# 向后递归
execute positioned ^ ^ ^-0.25 if function teleport:if/through run function teleport:backward/ray
# 声音和消息
